<template>
	<df-result-layout :collection-length="collection.length">
		<v-card variant="outlined" border="sm opacity-20" class="mb-4" v-for="brand in collection" :key="brand.identity">
			<v-card-title class="d-flex align-center">
				...
				<v-menu>
					...
					<v-list width="150">
						<v-list-item @click="$emit('accessEdition', brand)" append-icon="mdi-file-document-edit-outline">
							<v-list-item-title>Edit</v-list-item-title>
						</v-list-item>
						<v-list-item @click="$emit('executeExclusion', brand)" append-icon="mdi-trash-can-outline">
